...
//  Blocstagram
//
//  Created by Yong Jun on 6/18/15.
...
- (instancetype)initWithImage:(UIImage *)image {
    ...
    if (self) {
        ...
        self.filterImages = [NSMutableArray arrayWithObject:image];
        self.filterTitles = [NSMutableArray arrayWithObject:NSLocalizedString(@"None", @"Label for when no filter is applied")];
...
- (void) sendButtonPressed:(UIButton *)sender {
    ...
    if ([[UIApplication sharedApplication] canOpenURL:instagramUrl]) {
        alertVC = [UIAlertController alertControllerWithTitle:@"" message:@"Add a cpation and send your image in the Instagram app" preferredStyle:UIAlertControllerStyleAlert];
        ...
        [alertVC addAction:[UIAlertAction actionWithTitle:@"Send" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            UITextField *textField = alertVC.textFields[0];
            [self sendImageToInstagramWithCaption:textField.text];
        }]];
    } else {
        alertVC = [UIAlertController alertControllerWithTitle:@"No Instagram App" message:@"Add a caption and send your image in the Instagram app" preferredStyle:UIAlertControllerStyleAlert];
...
- (void) sendImageToInstagramWithCaption:(NSString *)caption {
    ...
    if (!success) {
        UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"Couldn't save image" message:@"Not enough disk space" preferredStyle:UIAlertControllerStyleAlert];
        [alertVC addAction:[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:nil]];
        [self presentViewController:alertVC animated:YES completion:nil];
        return;
    }
     ...
     if (self.sendButton.superview) {
         ...
         [self.documentController presentOptionsMenuFromBarButtonItem:self.sendBarButton animated:YES];
     }
}
